 ...
 *
 * and has been adapted here by Lluis Garrido, September 2016.
 *
 */
...
static void freeRBData(RBData *data) {
    deleteList(data->aeropuertosDestino);
    free(data->aeropuertosDestino);
    free(data->key);
    free(data);
}
...
void insertNode(RBTree *tree, RBData *data) {
    ...
    while (current != NIL) {
        if (compEQ(data->key, current->data->key)) {
            printf(
                    "insertNode: trying to insert but primary key is already in tree.\n");
            exit(1);
        }
        parent = current;
        current =
                compLT(data->key, current->data->key) ?
